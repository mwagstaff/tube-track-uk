...
struct MapMorphTransitionLayer: View, @preconcurrency Animatable {
    ...
    var body: some View {
        GeometryReader { proxy in
            if let camera = appState.beckMapCameraSnapshot,
    ...
    private func drawNetwork(
        ...
        for train in appState.liveTrains {
            ...
            let marker = Path(roundedRect: rect, cornerRadius: 6)
            context.fill(marker, with: .color(.tubeLine(train.lineID)))
            context.stroke(marker, with: .color(.white), lineWidth: 1.5)
            context.draw(icon, in: rect.insetBy(dx: 4.5, dy: 4.5))
        }
    }
