...
def build(svg_path: Path, graph_path: Path, resources: Path) -> dict:
    ...
    # Hayes & Harlington is one Elizabeth line junction, not a passenger
    # interchange between separate platforms. The two official trace fragments
    # land a few units apart at the branch seam; coalesce every semantic port at
    # their midpoint and draw one shared roundel over the joined route paths.
    hayes_station_id = "910GHAYESAH"
    hayes_port = line_port(selected_segments, hayes_station_id, "elizabeth")
    set_line_port(hayes_station_id, "elizabeth", hayes_port)
    replace_marker(hayes_station_id, hayes_port, [circle(hayes_port)])

    # The Richmond branch runs beside the Mildmay line at these three stations.
    # Project both traced ports onto their shared route position so the
    # interchange bar is exactly perpendicular to the parallel 45-degree
    # routes. This matches the official map's 45-degree paired-roundel axis
    # while moving each source port by the smallest possible distance.
    for district_station_id, mildmay_station_id in (
        ("940GZZLURMD", "910GRICHMND"),
        ("940GZZLUKWG", "910GKEWGRDN"),
        ("940GZZLUGBY", "910GGNRSBRY"),
    ):
        district_port = line_port(selected_segments, district_station_id, "district")
        mildmay_port = line_port(selected_segments, mildmay_station_id, "mildmay")
        shared_route_position = (
            district_port[0] - district_port[1]
            + mildmay_port[0] - mildmay_port[1]
        ) / 2

        def aligned_port(port: vector.Point) -> vector.Point:
            route_axis = port[0] + port[1]
            return (
                (route_axis + shared_route_position) / 2,
                (route_axis - shared_route_position) / 2,
            )

        aligned_district_port = aligned_port(district_port)
        aligned_mildmay_port = aligned_port(mildmay_port)
        move_line_port(district_station_id, "district", aligned_district_port)
        move_line_port(mildmay_station_id, "mildmay", aligned_mildmay_port)
        replace_marker(
            district_station_id,
            aligned_district_port,
            [circle(aligned_district_port)],
        )
        replace_marker(
            mildmay_station_id,
            aligned_mildmay_port,
            [
                connector(aligned_district_port, aligned_mildmay_port),
                circle(aligned_mildmay_port),
            ],
        )
        label = labels_by_station_id.get(district_station_id)
        if label is not None:
            previous_label_position = (
                label["position"]["x"],
                label["position"]["y"],
            )
            place_label(
                ...
                aligned_district_port,
                (
                    previous_label_position[0] - district_port[0],
                    previous_label_position[1] - district_port[1],
                ),
                label["alignment"],
            )
    ...
    original_blackhorse_overground = line_port(
        selected_segments, "910GBLCHSRD", "suffragette"
    )
    # The Suffragette line leaves Blackhorse Road on a southeast 45-degree
    # diagonal. Project its roundel along that route until the interchange bar
    # is the matching northeast 45-degree diagonal from the Victoria roundel.
    overground_route_axis = (
        original_blackhorse_overground[0]
        - original_blackhorse_overground[1]
    )
    interchange_axis = blackhorse_victoria[0] + blackhorse_victoria[1]
    blackhorse_overground = (
        (overground_route_axis + interchange_axis) / 2,
        (interchange_axis - overground_route_axis) / 2,
    )
    move_line_port("910GBLCHSRD", "suffragette", blackhorse_overground)
    replace_marker(
        "940GZZLUBLR",
        blackhorse_victoria,
    ...
    # Finchley Central is the junction below the Mill Hill East spur, not a
    # separate tick above the branch join. Coalesce all three Northern line
    # endpoints on the branch's existing junction so that vertical order is
    # unambiguous and the main High Barnet route stays continuous.
    mill_hill_segment = selected_segments[
        "northern:940GZZLUFYC:940GZZLUMHL"
    ]
    finchley_branch_endpoint = (
        mill_hill_segment["toPort"]["x"],
        mill_hill_segment["toPort"]["y"],
    )
    set_line_port("940GZZLUFYC", "northern", finchley_branch_endpoint)
    replace_marker(
        "940GZZLUFYC",
        finchley_branch_endpoint,
        [tick_at("northern", finchley_branch_endpoint, (0, 1))],
    )
    place_label(
        "940GZZLUFYC",
        finchley_branch_endpoint,
    ...
    # Keep Caledonian Road, Holloway Road and Arsenal on the Piccadilly line's
    # x+y=3437.578 diagonal, above the Mildmay corridor as on the official map.
    # Caledonian Road is the perpendicular projection of its Overground partner
    # onto that diagonal, producing a short 45-degree interchange. Distribute
    # the next two stops evenly towards Finsbury Park so the station order and
    # visual rhythm remain intact.
    caledonian_overground = line_port(
        selected_segments, "910GCLDNNRB", "mildmay"
    )
    existing_holloway_piccadilly = line_port(
        selected_segments, "940GZZLUHWY", "piccadilly"
    )
    piccadilly_diagonal = sum(existing_holloway_piccadilly)
    overground_cross_axis = caledonian_overground[0] - caledonian_overground[1]
    caledonian_piccadilly = (
        (piccadilly_diagonal + overground_cross_axis) / 2,
        (piccadilly_diagonal - overground_cross_axis) / 2,
    )
    finsbury_park_piccadilly = line_port(
        selected_segments, "940GZZLUFPK", "piccadilly"
    )

    def piccadilly_station_between(fraction: float) -> vector.Point:
        return tuple(
            caledonian_piccadilly[index]
            + (finsbury_park_piccadilly[index] - caledonian_piccadilly[index])
            * fraction
            for index in (0, 1)
        )

    holloway_piccadilly = piccadilly_station_between(1 / 3)
    arsenal_piccadilly = piccadilly_station_between(2 / 3)
    for station_id, port in (
        ("940GZZLUCAR", caledonian_piccadilly),
        ("940GZZLUHWY", holloway_piccadilly),
        ("940GZZLUASL", arsenal_piccadilly),
    ):
        move_line_port(station_id, "piccadilly", port)
    ...
    replace_marker(
        "940GZZLUHWY",
        holloway_piccadilly,
        [tick_at("piccadilly", holloway_piccadilly, (1, -1))],
    )
    replace_marker(
        "940GZZLUASL",
        arsenal_piccadilly,
        [tick_at("piccadilly", arsenal_piccadilly, (1, -1))],
    )
    replace_marker(
        "910GCLDNNRB",
    ...
    place_label("940GZZLUHWY", holloway_piccadilly, (15, -10), "leading")
    place_label("940GZZLUASL", arsenal_piccadilly, (15, -16), "leading")
    ...
    # King's Cross uses a shared deep-level node and a separate sub-surface
    # node to its southwest. Preserve the three-lane spacing while shifting the
    # Circle, H&C and Metropolitan bend left, then join the two roundels with
    # the official southeast-to-northwest 45-degree interchange bar.
    kings_cross_station_id = "940GZZLUKSX"
    kings_cross_deep = line_port(
        selected_segments, kings_cross_station_id, "piccadilly"
    )
    for line_id in ("northern", "victoria"):
        move_line_port(kings_cross_station_id, line_id, kings_cross_deep)

    kings_cross_lines = ("circle", "hammersmith-city", "metropolitan")
    old_kings_cross_shared = shared_line_port(
        selected_segments, kings_cross_station_id, kings_cross_lines
    )
    roundel_spacing = old_kings_cross_shared[1] - kings_cross_deep[1]
    kings_cross_shared = (
        kings_cross_deep[0] - roundel_spacing,
        kings_cross_deep[1] + roundel_spacing,
    )
    shared_shift_x = kings_cross_shared[0] - old_kings_cross_shared[0]
    for line_id in kings_cross_lines:
        old_line_port = line_port(
            selected_segments, kings_cross_station_id, line_id
        )
        move_line_port(
            kings_cross_station_id,
            line_id,
            (old_line_port[0] + shared_shift_x, old_line_port[1]),
        )

        # Move the complete bend, not only its station endpoint. This retains
        # the official smooth transition from the Euston Square horizontal to
        # the Farringdon diagonal without leaving a small westward hook.
        inbound_segment = selected_segments[
            f"{line_id}:940GZZLUESQ:{kings_cross_station_id}"
        ]
        inbound_commands = selected_paths[inbound_segment["pathID"]]["commands"]
        for command in inbound_commands[1:-1]:
            for key in ("to", "control1", "control2"):
                if key in command:
                    command[key] = translated_point(
                        command[key], shared_shift_x, 0
                    )
    ...
    replace_marker(
        ...
        kings_cross_deep,
        [
            connector(kings_cross_deep, kings_cross_shared),
            circle(kings_cross_deep),
            circle(kings_cross_shared),
        ],
    )

    # The Circle, H&C, and Metropolitan lanes through Farringdon form one
    # unbroken diagonal between King's Cross and Barbican. Preserve each
    # lane's vertical position and solve its horizontal position on that
    # straight chord, removing the inherited kink without disturbing either
    # adjoining station. The Elizabeth roundel then slides along its horizontal
    # route to retain the compact 45-degree pedestrian link.
    farringdon_lines = ("circle", "hammersmith-city", "metropolitan")
    for line_id in farringdon_lines:
        kings_cross_port = line_port(
            selected_segments, "940GZZLUKSX", line_id
        )
        old_farringdon_port = line_port(
            selected_segments, "940GZZLUFCN", line_id
        )
        barbican_port = line_port(
            selected_segments, "940GZZLUBBN", line_id
        )
        corridor_dy = barbican_port[1] - kings_cross_port[1]
        if abs(corridor_dy) < 0.001:
            raise ValueError(f"Unexpected flat {line_id} Farringdon corridor")
        progress = (
            (old_farringdon_port[1] - kings_cross_port[1]) / corridor_dy
        )
        straight_farringdon_x = kings_cross_port[0] + progress * (
            barbican_port[0] - kings_cross_port[0]
        )
        move_line_port(
            "940GZZLUFCN",
            line_id,
            (straight_farringdon_x, old_farringdon_port[1]),
        )
    ...
    farringdon_elizabeth = (
        farringdon_shared[0]
        - (farringdon_elizabeth[1] - farringdon_shared[1]),
        farringdon_elizabeth[1],
    )
    move_line_port("910GFRNDXR", "elizabeth", farringdon_elizabeth)
    replace_marker(
        "910GFRNDXR", farringdon_elizabeth, [circle(farringdon_elizabeth)]
    )
    ...
    # Liverpool Street uses one upper Elizabeth node and one lower node shared
    # by Central, Circle, H&C and Metropolitan. This keeps the Central route on
    # the lower interchange instead of pulling it north to meet Elizabeth. The
    # Elizabeth node also retains the official 45-degree passage to Moorgate's
    # Northern line roundel at the western end of the station.
    liverpool_marker = markers_by_id["940GZZLULVT"]
    moorgate_northern = min(
        primitive_points(markers_by_id["940GZZLUMGT"]),
        key=lambda point_value: point_value[1],
    )
    old_liverpool_elizabeth = line_port(
        selected_segments, "910GLIVSTLL", "elizabeth"
    )
    liverpool_elizabeth = (
        moorgate_northern[0]
        + (moorgate_northern[1] - old_liverpool_elizabeth[1]),
        old_liverpool_elizabeth[1],
    )
    for station_id in ("910GLIVST", "910GLIVSTLL"):
        move_line_port(station_id, "elizabeth", liverpool_elizabeth)
    liverpool_weaver = line_port(selected_segments, "910GLIVST", "weaver")
    replace_marker(
        "910GLIVST",
        liverpool_elizabeth,
        [
            connector(liverpool_elizabeth, liverpool_weaver),
            circle(liverpool_weaver),
        ],
    )

    old_liverpool_shared = shared_line_port(
        selected_segments,
        "940GZZLULVT",
        ("circle", "hammersmith-city", "metropolitan"),
    )
    shared_shift_x = liverpool_elizabeth[0] - old_liverpool_shared[0]
    for line_id in ("circle", "hammersmith-city", "metropolitan"):
        old_line_port = line_port(selected_segments, "940GZZLULVT", line_id)
        move_line_port(
            "940GZZLULVT",
            line_id,
            (old_line_port[0] + shared_shift_x, old_line_port[1]),
    ...
    move_line_port("940GZZLULVT", "central", liverpool_shared)
    replace_marker(
        "910GLIVSTLL",
        liverpool_elizabeth,
    ...
    liverpool_marker["primitives"] = [
        connector(liverpool_elizabeth, liverpool_shared),
        connector(liverpool_elizabeth, moorgate_northern),
        circle(liverpool_elizabeth),
        circle(liverpool_shared),
    ]
    liverpool_marker["anchor"] = vector.rounded(liverpool_shared)
    ...
    interchange_axis = monument_roundel[1] - monument_roundel[0]
    bank_central = (
        old_bank_central[1] - interchange_axis,
        old_bank_central[1],
    )
    ...
    bank_northern = (
        old_bank_northern[0],
        bank_central[1] + old_bank_northern[0] - bank_central[0],
    )
    move_line_port("940GZZLUBNK", "northern", bank_northern)
    move_line_port("940GZZDLBNK", "dlr", bank_northern)

    replace_marker(
        "940GZZLUBNK",
        bank_central,
        [
            connector(bank_central, bank_northern),
            circle(bank_central),
            circle(bank_northern),
        ],
    )
    dlr_bank_marker = markers_by_id["940GZZDLBNK"]
    dlr_bank_marker["anchor"] = vector.rounded(bank_northern)
    dlr_bank_marker["primitives"] = [
        connector(bank_northern, monument_roundel),
        circle(bank_northern),
    ]
